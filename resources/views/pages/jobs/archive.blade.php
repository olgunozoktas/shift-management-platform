...
@section('content')
    ...
    <section class="flex flex-col gap-2">
        <header class="flex flex-row items-center justify-between border-b border-gray-200 pb-4">
            <h1 class="text-black font-black text-lg">Archived Jobs</h1>
                        ...
                        <tr class="text-center bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="py-2">
                                @if($job->employee_time_cards_count > 0)
                                    <a href="{{ route('jobs.archive.employee-time-cards.show', $job->id) }}"
                                       class="py-2 px-4 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-lg">Show</a>
                                @else
                                    Not found
                                @endif
                            </td>
                            <td class="py-2">{{ \Carbon\Carbon::parse($job->deleted)->format('Y-m-d H:i') }}</td>
                        </tr>
                    @endforeach
                @else
                    <tr class="text-center">
                        <td colspan="4" class="py-4">No Archived Jobs Found In System</td>
                    </tr>
                @endif
                </tbody>
            </table>
