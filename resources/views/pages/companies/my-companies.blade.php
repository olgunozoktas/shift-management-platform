...
@section('content')
    ...
    <section class="flex flex-col gap-2">
        <header class="flex flex-row items-center justify-start border-b border-gray-200 pb-4">
            <h1 class="text-black font-black text-lg">My Companies</h1>
        </header>
                ...
                @if(count($companies) > 0)
                    @foreach($companies as $company)
                        <tr class="text-center bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <th scope="row" class="w-1/4 px-6 py-4 font-medium text-gray-900 dark:text-white">
                                {{ ucfirst($company->company_role) }}
                            </th>
                            <td class="flex flex-row items-center justify-center gap-2 py-2">
                                <a href="{{ route('company-users.show', $company->id) }}"
                                   class="py-2 px-4 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-lg">Users</a>
                            </td>
                        </tr>
                    @endforeach
                @else
                    <tr class="text-center">
                        <td colspan="4" class="py-4">No Companies Assigned You Yet</td>
                    </tr>
                @endif
                </tbody>
            </table>
