        ...
        <section class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" id="table">
                ...
                <tr class="text-center bg-gray-100">
                    ...
                    <th scope="col" class="px-6 py-3">Answered Forms</th>
                ...
                @if(count($assignedLocations) > 0)
                    @foreach($assignedLocations as $location)
                        <tr class="text-center bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <th scope="row" class="w-1/4 px-6 py-4 font-medium text-gray-900 dark:text-white">
                                {{ count($location->location->locationForms) }}
                            </th>
                            <th scope="row" class="w-1/4 px-6 py-4 font-medium text-gray-900 dark:text-white">
                                {{ count($location->location->locationForms) }}
                            </th>
                            <td class="flex flex-row items-center justify-center gap-2 py-2">
                                @if(count($location->location->folders))
                                    <a href="{{ route('assigned-location.files', $location->location->id) }}"
                                       class="py-2 px-4 bg-green-500 hover:bg-green-600 text-white font-bold rounded-lg">Show
                                        Files</a>
                                @endif

                                <a href="{{ route('assigned-location.forms', $location->location->id) }}"
                                   class="py-2 px-4 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-lg">Show
                                    Forms</a>
                            </td>
                        </tr>
                    @endforeach
