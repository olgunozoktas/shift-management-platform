...
@section('content')
    ...
    <section class="flex flex-col gap-2">
        <header class="flex flex-row items-center justify-between border-b border-gray-200 pb-4">
            <h1 class="text-black font-black text-lg">All Question Forms</h1>
            <a href="{{ route('question-forms.create') }}"
               class="border-2 border-gray-600 bg-gray-800 hover:bg-gray-900 text-white font-black py-2 px-4 rounded-full">
                Create New Question Forms
            </a>
        </header>
        ...
        <section class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" id="table">
                ...
                <tr class="text-center bg-gray-100">
                    ...
                    <th scope="col" class="px-6 py-3">Total Sections</th>
                    <th scope="col" class="px-6 py-3">Total Questions</th>
                    <th scope="col" class="px-6 py-3">Assigned Locations / Facilities</th>
                ...
                @if(count($questionForms) > 0)
                    @foreach($questionForms as $questionForm)
                        <tr class="text-center bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <th scope="row" class="w-1/4 px-6 py-4 font-medium text-gray-900 dark:text-white">
                                {{ $questionForm->sections_count }}
                            </th>
                            <th scope="row" class="w-1/4 px-6 py-4 font-medium text-gray-900 dark:text-white">
                                {{ $questionForm->questions_count }}
                            </th>
                            <th scope="row" class="w-1/4 px-6 py-4 font-medium text-gray-900 dark:text-white">
                                {{ $questionForm->location_forms_count }}
                            </th>
                            <td class="flex flex-row items-center justify-center gap-2 py-2">
                                <a href="{{ route('question-forms.edit', $questionForm->id) }}"
                                   class="py-2 px-4 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-lg">Edit</a>
                                <button onclick="deleteQuestionForm({{ $questionForm->id }})"
                ...
                @else
                    <tr class="text-center">
                        <td colspan="4" class="py-4">No Question Forms Found In System</td>
                    </tr>
                @endif
                </tbody>
            </table>
...
@push('js')
    ...
    <script>
        function deleteQuestionForm(id) {
            if (confirm('Would you like to delete this question form?')) {
                let route = '{{ route('question-forms.delete', ':id') }}';
