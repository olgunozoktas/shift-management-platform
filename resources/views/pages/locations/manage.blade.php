...
@section('content')
    ...
    <section class="flex flex-col gap-2">
        <header class="flex flex-row items-center justify-between border-b border-gray-200 pb-4">
            <h1 class="text-black font-black text-lg">{{ !$isEdit ? 'Create Location Page' : 'Edit Location Page' }}</h1>
        ...
        <form action="{{ $route }}" class="flex flex-col gap-4 mt-4" method="POST">
            ...
            <label class="block">
                ...
                <input type="text" name="name" value="{{ old('name', isset($location) ? $location->name : '') }}"
            ...
            <label class="block">
                <span class="text-gray-700">Form Selector</span>
                <select name="question_forms[]" multiple id="question_forms"
                        class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    @foreach($questionForms as $questionForm)
                        <option value="{{ $questionForm->id }}" @if(isset($location) && in_array($questionForm->id, $locationForms)) selected @endif>{{ $questionForm->description }}</option>
                    @endforeach
                </select>
            </label>
