...
@section('content')
    ...
    <section class="flex flex-col gap-2">
        <header class="flex flex-row items-center justify-start border-b border-gray-200 pb-4">
            <h1 class="text-black font-black text-lg">My Schedules</h1>
        </header>

        <div id="my-schedules"></div>
    </section>
@endsection
...
@push('js')
    @if(config('app.env') == 'production')
        ...
        @endphp
        <script type="module" src="{{ asset("build/{$manifest['resources/js/my-schedules.jsx']['file']}") }}"></script>
    @else
        @vitereactrefresh
        @vite('resources/js/my-schedules')
    @endif
@endpush
